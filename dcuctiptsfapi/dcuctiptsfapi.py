...
#
class Config:
    def __init__(self, ctipApi, subscriptionName, subscriptionKey, dataFileTimestamp, daysAgo = 14, saveCtipDataFiles= False):
        ...
        self.DaysAgo = daysAgo                      # The timeframe to retrieve data from the CTIP API -- valid values are 1..180
...
def CtipApi(config: Config) -> list:
    ...
    try:
        ...
        log.debug(f'           Timespan (hours): {config.DaysAgo}')
        ...
        while True:
            # Setup request URL
            apiUrl = f"{CTIP_API_BASE_URL}/{config.CtipApi.lower()}?daysago={config.DaysAgo}&offset={offset}"
            ...
            # Check for 400 response
                ...
                log.error(f'{BASE_SPACE} !!!> Encountered 400 error. Invalid value for the daysago API parameter. Valid values are 1..180.')
...
def ProcessCtipData(ctipData: list, config: Config) -> int:
    ...
    if (config.CtipApi==CTIP_API_FRAUD_TSF):
        ...
        for objCtipData in ctipData:        
            ...
            log.info(f'{BASE_SPACE} [{itemCount:07d} / {downloadedCtipItems:07d}] Operation: {objCtipData["Operation"]} // ThreatCode: {objCtipData["ThreatCode"]} // Report ID: {objCtipData["ReportID"]}')
            #
            # *******************************************************************************************
            # *******************************************************************************************
            # *******************************************************************************************
...
def main():
    ...
    parser.add_argument('--subscription-name', '-sn', default='dcuctiptsfapi', help='Used to name the downloaded data file(s)\nDefault setting is "dcuctiptsfapi"')
    parser.add_argument('--days-ago', '-da', type=int, default=14, help='The timespan in days to query historical CTIP API data\nRange of acceptable values is 1..180\nDefault setting is 14')
    ...
    try:
        ...
        objConfigTsf = Config(ctipApi=CTIP_API_FRAUD_TSF,           # The target CTIP API endpoint -- use constant CTIP_API_FRAUD_TSF
                              ...
                              daysAgo=daysAgo,                      # The timeframe to retrieve data from the CTIP API -- valid values are 1..180
                              saveCtipDataFiles=saveCtipDataFiles)  # Flag to control CTIP data file creation
                              
        ctipTsfDataItems = CtipApi(config=objConfigTsf)
